...
import { 
  Search, Sparkles, ArrowRight, Star, Bookmark, BookmarkCheck, LayoutGrid, Gift, ChevronDown, Check, Target, User, Lightbulb, X
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { supabase } from "@/integrations/supabase/client"; 

// 🔥 QUIZ COMPONENT (Unaffected by main page CSS)
const QuizOverlay = ({ 
  onComplete, 
  onSkip 
}: { 
  ...
  return (
    <div className="fixed inset-0 z-[999] bg-[#F9FAFB] flex flex-col justify-center items-center p-6 animate-in fade-in duration-500">
      ...
      <div className="w-full max-w-xl flex flex-col">
          ...
          <div className="animate-in slide-in-from-bottom-4 duration-500 w-full">
            ...
            <div className="space-y-4">
              {["Make Money", "Save Time", "Just Exploring"].map((opt) => (
                <button 
                  key={opt}
                  onClick={() => handleAnswer("goal", opt, 1)}
          ...
          <div className="animate-in slide-in-from-right-8 duration-500 w-full">
            ...
            <div className="space-y-4">
              {["Content Creator", "Developer / Student", "Business Owner"].map((opt) => (
                <button 
                  key={opt}
                  onClick={() => handleAnswer("role", opt, 2)}
                  className="w-full p-5 text-left bg-white border-2 border-gray-100 rounded-2xl hover:border-indigo-500 hover:bg-indigo-50 text-gray-800 font-bold text-lg transition-all active:scale-[0.98] shadow-sm hover:shadow-md"
                >
                  {opt}
                </button>
        ...
        {step === 3 && (
          <div className="animate-in zoom-in-95 duration-500 w-full bg-white p-8 md:p-10 rounded-3xl shadow-2xl border border-gray-100 text-center">
            <div className="inline-flex p-4 rounded-full bg-emerald-100 text-emerald-600 mb-6">
              <Sparkles className="w-8 h-8" />
            </div>
            <h2 className="text-3xl font-black text-gray-900 mb-3 tracking-tight">Your Toolkit is Ready!</h2>
            <p className="text-gray-500 mb-8 max-w-sm mx-auto">
              Enter your email to receive your personalized AI roadmap and daily tool updates.
            </p>
            <div className="flex flex-col gap-4">
              ...
              <button 
                ...
                className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-lg rounded-xl shadow-[0_10px_30px_rgba(79,70,229,0.3)] transition-all hover:-translate-y-1 active:scale-95 flex justify-center items-center gap-2"
              >
                Show My Tools <ArrowRight className="w-5 h-5" />
              </button>
            </div>
          </div>
        )}
      ...
      {step < 3 && (
        <button 
          onClick={onSkip}
          className="absolute bottom-8 text-gray-400 font-medium hover:text-gray-600 transition-colors flex items-center gap-1"
        >
...
const Index = () => {
  ...
  // 🔥 STATE FOR QUIZ
  const [quizFilterTag, setQuizFilterTag] = useState<string | null>(null);
  ...
  const handleQuizComplete = (tag: string) => {
  ...
  const filtered = useMemo(() => {
    ...
    // 4. Apply Custom Pricing
    if (pricingFilter !== "All") {
      result = result.filter((t) => t.pricing && t.pricing.toLowerCase() === pricingFilter.toLowerCase());
    }

    // 🔥 5. INVISIBLE TAG FILTER FROM QUIZ
    if (quizFilterTag) {
      result = result.filter((t) => {
        if (!t.tags) return false; 
        return t.tags.toLowerCase().includes(quizFilterTag.toLowerCase());
      });
    }
  ...
  return (
    <>
      {showQuiz && (
        <QuizOverlay onComplete={handleQuizComplete} onSkip={handleQuizSkip} />
      )}

      {/* 🔥 REMOVED THE BREAKING CSS CLASSES HERE */}
      <div className="min-h-screen bg-[#050505] text-foreground transition-all duration-700">
          ...
          <div className="flex w-full justify-between md:justify-start gap-2 md:gap-4 border-b border-white/10 pb-4">
            <button 
              ...
              className={`group flex flex-col md:flex-row items-center justify-center gap-1.5 md:gap-2 flex-1 md:flex-none py-3 md:px-6 rounded-2xl md:rounded-xl transition-all duration-300 active:scale-95 ${
                activeTab === "all" ? "bg-white/10 text-white border border-white/20 shadow-[0_0_20px_rgba(255,255,255,0.05)] translate-y-0" : "text-gray-500 bg-transparent hover:text-white hover:bg-white/5 hover:-translate-y-1"
              }`}
            >
            ...
            <button 
              ...
              className={`group flex flex-col md:flex-row items-center justify-center gap-1.5 md:gap-2 flex-1 md:flex-none py-3 md:px-6 rounded-2xl md:rounded-xl transition-all duration-300 active:scale-95 ${
                activeTab === "free" ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.2)] translate-y-0" : "text-gray-500 bg-transparent hover:text-white hover:bg-white/5 hover:-translate-y-1"
              }`}
            >
            ...
            <button 
              ...
              className={`group flex flex-col md:flex-row items-center justify-center gap-1.5 md:gap-2 flex-1 md:flex-none py-3 md:px-6 rounded-2xl md:rounded-xl transition-all duration-300 active:scale-95 ${
                activeTab === "saved" ? "bg-primary/20 text-primary border border-primary/30 shadow-[0_0_20px_rgba(var(--primary),0.2)] translate-y-0" : "text-gray-500 bg-transparent hover:text-white hover:bg-white/5 hover:-translate-y-1"
              }`}
            >
              ...
              {visibleRest.length > 0 ? (
                <section>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-10">
                    ...
                    {/* BUTTON TO CLEAR QUIZ FILTER */}
                    {quizFilterTag && (
                      <Button onClick={clearQuizFilter} variant="outline" className="border-white/10 text-gray-400 hover:text-white">
                        Clear Matchmaker Filter <X className="ml-2 w-4 h-4" />
                      </Button>
                    )}
                  </div>
                  <ToolGrid 
                    tools={visibleRest} 
                    isSearch={!!search || !!quizFilterTag} 
                    checkActive={isSponsorshipActive} 
                    onVisit={trackClick}
                    savedIds={savedToolIds} 
                    onToggleSave={toggleSaveTool} 
                ...
                <div className="text-center py-20 border border-dashed border-white/10 rounded-3xl bg-white/5 transition-all duration-500 animate-in fade-in slide-in-from-bottom-4">
                  ...
                  <Button onClick={clearQuizFilter} className="mt-6 font-bold bg-primary text-white transition-transform hover:scale-105">View All Tools</Button>
                </div>
              )}
